...
>>>>>>> upstream/master
        ...
        float maxDistance)
    {
        ...
        var maxDistanceSq = (int)(maxDistance * maxDistance); // Forge-Change
        ...
        foreach (var gridToTransform in localGrids)
        {
            ...
            var (_, gridWorldRotation, gridWorldMatrix, invGridWorldMatrid) = _transformSystem.GetWorldPositionRotationMatrixWithInv(xform, xforms); // Forge-Change

            var localEpicentre = (Vector2i)Vector2.Transform(epicentre.Position, invGridWorldMatrid); // Forge-Change
            ...
            foreach (var (tile, dir) in edges)
            {
                ...
                if ((dir & NeighborFlag.Cardinal) == 0)
                {
                    ...
                    var newIndex = new Vector2i((int)MathF.Floor(center.X), (int)MathF.Floor(center.Y)); // Forge-Change
